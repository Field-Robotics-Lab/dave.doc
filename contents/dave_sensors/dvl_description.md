...
See [whn_dvl_examples](/dave/dave_sensors/whn_dvl_examples) for a complete description of the plugin's use including working examples.
...
The plugin must be associated with a Gazebo model.  The following example is excerpted from a URDF model generated by the Xacro templates discussed in the [DVL Examples](/dave/dave_sensor/whn_dvl_examples) tutorial:
...
The plugin must be associated with a Gazebo sensor.  The following example is excerpted from a URDF model generated by the Xacro templates discussed in the [tutorial](/dave/dave_sensors/whn_dvl_examples).
...
Current-aware water tracking is compatible with the UUV Simulator [ocean current plugin](/dave/dave_env/Ocean-Current) (or any Gazebo plugin that publishes to the ocean current topic).  The plugin subscribes to the specified ocean current topic and interprets received `gazebo::msgs::Vector3d` messages as a current velocity vector in Gazebo's ENU frame.
